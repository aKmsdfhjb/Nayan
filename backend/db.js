...
export async function initDb() {
  ...
  if (Number(profileResult.rows[0].count) === 0) {
    await db.execute({
      sql: `INSERT INTO profile (id, name, title, tagline, location, email, linkedin, bio)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        "primary",
        "Nayan Kuikel",
        "Civil Engineer",
        "Structural precision, cost intelligence, and dependable site judgment.",
        "Kathmandu, Bāgmatī, Nepal",
        "nayan@email.com",
        "linkedin.com/in/nayan-kuikel",
        "Civil Engineer specializing in structural inspection and cost estimation. Skilled in analyzing project requirements, assessing costs, preparing detailed estimates, and ensuring structural integrity. Strong communicator and team leader with experience across inspection, reporting, and field coordination.",
      ],
    });
  }
  ...
  if (Number(projectResult.rows[0].count) === 0) {
    const insertProject = `INSERT INTO projects (id, title, category, description, tools, year, image, link) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    await db.execute({
      sql: insertProject,
      args: [
        randomUUID(),
        ...
        JSON.stringify(["AutoCAD", "Field Inspection"]),
        "2024",
        "/blueprint-1.svg",
        null,
      ],
    ...
    await db.execute({
      ...
      args: [
        randomUUID(),
        "Repair Cost Estimation - Commercial Property",
        "Estimation",
        ...
        JSON.stringify(["AutoCAD", "Microsoft Excel"]),
        "2024",
        "/blueprint-2.svg",
        null,
      ],
    ...
    await db.execute({
      ...
      args: [
        randomUUID(),
        "Structural Quality Control Report",
        "Structural",
        ...
        JSON.stringify(["Field Inspection", "Report Writing"]),
        "2024",
        "/blueprint-3.svg",
        null,
      ],
  ...
  if (Number(expResult.rows[0].count) === 0) {
    await db.execute({
      sql: `INSERT INTO experience (id, role, company, duration, description, skills) VALUES (?, ?, ?, ?, ?, ?)`,
      args: [
        randomUUID(),
        "Estimator",
        "SkillSewa Pvt. Ltd.",
        ...
        JSON.stringify(["Cost Estimation", "Quantity Takeoff", "AutoCAD", "Site Assessment"]),
      ],
    });
  }
  ...
  if (Number(adminResult.rows[0].count) === 0) {
    ...
    await db.execute({
      sql: `INSERT INTO admin_users (id, username, password_hash, must_change_password) VALUES (?, ?, ?, 1)`,
      args: [randomUUID(), "admin", hash],
    });
  }
}
